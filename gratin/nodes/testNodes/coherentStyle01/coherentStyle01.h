...
class CoherentStyleWidget : public NodeWidget {
 public:
 CoherentStyleWidget(NodeInterface *node) :
  NodeWidget(node),
...
class CoherentStyleNode : public NodeTexture2D {
 public:
  CoherentStyleNode(PbGraph *parent=NULL,NodeHandle *handle=NULL);
  ~CoherentStyleNode();
  void apply();
...
class CoherentStyleHandle : public QObject, public NodeHandleInterface {
  Q_OBJECT
    ...
    Q_PLUGIN_METADATA(IID "Gratin.CoherentStyle")

    public:
  const QString     stringId() const {return "coherentStyle01Id";}
  unsigned int      version () const {return 1;}

  const QString     name    () const {return "coherentStyle01";}
  ...
  NodeInterface *createInstance(PbGraph *parent) {
    return new CoherentStyleNode(parent,new NodeHandle(this));
  }
  ...
  NodeInterface *createInstance(PbGraph *parent,
    ...
    return new CoherentStyleNode(parent,new NodeHandle(stringId(),version(),name,path,desc,help,
