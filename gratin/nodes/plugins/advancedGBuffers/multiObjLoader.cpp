...
Mesh *MultiObjLoader::load(const string &filename, std::vector<Mesh *> &outMeshes) {

  if (isObj(filename)) {
    return loadObjFile(filename, outMeshes);
  }

  return NULL;
}

Mesh * MultiObjLoader::loadObjFile(const std::string &filename, std::vector<Mesh *> &outMeshes)
{
  Mesh *mesh = NULL;
  ...
  if (!loadObjTopology(filename, type, allVertIndices, allNormIndices,
                       allUVIndices, allMaterialIDs, numMaterialIDs))
    return NULL;
  ...
  mesh->computeNormals();
  allVert = mesh->getVertices();		// WTF?
  allNorm = mesh->getNormals();         // initialize normals?
  ...
  // split faces according to materialID
  for (int mi = 0; mi <= numMaterialIDs; ++mi) {
      Mesh* m = new Mesh();
      vector<Vector3i> mfaces;
      for (int i = 0; i < matIDs.size();++i) {
        if (matIDs[i] != mi)
            continue;
        mfaces.push_back(indices[i]);   // add face
      }
      if (mfaces.size() == 0) continue;
      printf("Output mesh with %i faces, id=%i\n", (int)mfaces.size(), mi);
...
bool MultiObjLoader::loadObjGeometry(const string &filename,
  ...
  while (1) {
    if ((returnValue = fscanf(fp, "%s", id)) == EOF)
      break;
...
bool MultiObjLoader::loadObjTopology(const string &filename, int type,
  ...
  while (1) {
    ...
    if (strcmp(id, "f") == 0 || strcmp(id, "fo") == 0) {
      ...
      case 0: // vertex only
        ...
        if (returnValue == 3) {
		  ...
		  materialIDs.push_back(curMatID);
        } else {
	...
	else if (strcmp(id, "g") == 0 || strcmp(id, "o") == 0) { // newobj / group
		curMatID++;
	}
	else {
      // skip line
  ...
  fclose(fp);
  maxMaterialID = curMatID;
  return true;
}

void MultiObjLoader::sphericalMapping(
    const std::vector<Vector3f> &vertices,
    const std::vector<Vector3i> &vertexIndices, std::vector<Vector2f> &uvs,
  ...
  for (unsigned int i = 0; i < vertices.size(); ++i) {
    ...
    const float r =
        max(min(c[2] / sqrt(c[0] * c[0] + c[2] * c[2]), 1.0f), -1.0f);
    Vector2f uv;
