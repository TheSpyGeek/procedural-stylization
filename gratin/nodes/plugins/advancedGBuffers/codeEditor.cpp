...
GLSLSyntaxHighLighter::GLSLSyntaxHighLighter(QTextDocument *document)
  ...
  _builtinList.setPattern(
      "\\b(abs|acos|acosh|all|any|asin|asinh|atan|atanh|atomicCounter|"
      "atomicCounterDecrement|atomicCounterIncrement|barrier|bitCount|"
      "bitfieldExtract|bitfieldInsert|bitfieldReverse|ceil|clamp|cos|cosh|"
      "cross|degrees|determinant|dFdx|dFdy|distance|dot|EmitStreamVertex|"
      "EmitVertex|EndPrimitive|EndStreamPrimitive|equal|exp|exp2|faceforward|"
      "findLSB|findMSB|floatBitsToInt|floatBitsToUint|floor|fma|fract|frexp|"
      "fwidth|greaterThan|greaterThanEqual|imageAtomicAdd|imageAtomicAnd|"
      "imageAtomicCompSwap|imageAtomicExchange|imageAtomicMax|imageAtomicMin|"
      "imageAtomicOr|imageAtomicXor|imageLoad|imageSize|imageStore|"
      ...
      "lessThan|lessThanEqual|log|log2|matrixCompMult|max|memoryBarrier|min|"
      "mix|mod|modf|noise|normalize|not|notEqual|outerProduct|packDouble2x32|"
      "packHalf2x16|packSnorm2x16|packSnorm4x8|packUnorm2x16|packUnorm4x8|pow|"
      "radians|reflect|refract|round|roundEven|sign|sin|sinh|smoothstep|sqrt|"
      "step|tan|tanh|texelFetch|texelFetchOffset|texture|textureGather|"
      ...
      "unpackUnorm4x8|usubBorrow)\\b");

  _typeList.setPattern(
      "\\b(float|double|int|void|bool|true|false|mat2|mat3|mat4|dmat2|dmat3|"
      "dmat4|mat2x2|mat2x3|mat2x4|dmat2x2|dmat2x3|dmat2x4|mat3x2|mat3x3|mat3x4|"
      "dmat3x2|dmat3x3|dmat3x4|mat4x2|mat4x3|mat4x4|dmat4x2|dmat4x3|dmat4x4|"
      ...
      "iimage2DMSArray|uimage2DMSArray|long|short|half|fixed|unsigned|hvec2|"
      "hvec3|hvec4|fvec2|fvec3|fvec4|sampler3DRect)\\b");

  _modifierList.setPattern(
      "\\b(attribute|const|uniform|varying|buffer|shared|coherent|volatile|"
      "restrict|readonly|writeonly|atomic_uint|layout|centroid|flat|smooth|"
      "noperspective|patch|sample|break|continue|do|for|while|switch|case|"
      "default|if|else|subroutine|in|out|inout|invariant|discard|return|lowp|"
      "mediump|highp|precision|struct|common|partition|active|asm|class|union|"
      "enum|typedef|template|this|packed|resource|goto|inline|noinline|public|"
      "static|extern|external|interface|superp|input|output|filter|sizeof|cast|"
      "namespace|using|row_major|early_fragment_tests)\\b");

  _preprocessList.setPattern("\\b(define|undef|if|ifdef|ifndef|else|elif|endif|"
...
void GLSLSyntaxHighLighter::modifyBlockFormat(const QString &text,
                                              QRegExp &regexp,
...
void GLSLSyntaxHighLighter::modifyComments(const QString &text, QRegExp &regexp,
                                           const QTextCharFormat &f) {
  int index = text.indexOf(regexp);
  if (index >= 0) {
...
CodeEditor::CodeEditor(QWidget *parent, bool readOnly, unsigned int lineShift)
    : QPlainTextEdit(parent), _lineShift(lineShift) {
...
void CodeEditor::lineNumberPaintEvent(QPaintEvent *event) {
  ...
  while (block.isValid() && top <= event->rect().bottom()) {
    if (block.isVisible() && bottom >= event->rect().top()) {
      ...
      painter.drawText(0, top, _lineNumberWidget->width(),
...
void CodeEditor::updateLineNumber(const QRect &rect, int dy) {
  if (dy)
    ...
    _lineNumberWidget->update(0, rect.y(), _lineNumberWidget->width(),
                              rect.height());
...
void CodeEditor::resizeEvent(QResizeEvent *e) {
  ...
  _lineNumberWidget->setGeometry(
      QRect(cr.left(), cr.top(), lineNumberWidth(), cr.height()));
}
