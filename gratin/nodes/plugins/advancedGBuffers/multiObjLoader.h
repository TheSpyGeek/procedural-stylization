...
#ifndef MULTI_OBJ_LOADER_H
#define MULTI_OBJ_LOADER_H
...
class MultiObjLoader {
public:
  static Mesh* load(const std::string &filename, std::vector<Mesh *> &outMeshes);

private:
  static Mesh* loadObjFile(const std::string &filename, std::vector<Mesh *> &outMeshes);
...
inline bool MultiObjLoader::isObj(const std::string &filename) {
  std::string tmp = std::string(filename.begin() + filename.find_last_of("."),
                                filename.end());
  return (tmp == ".obj" || tmp == ".OBJ");
}

inline Vector3f
MultiObjLoader::barycenter(const std::vector<Vector3f> &vertices) {
  Vector3f c(0, 0, 0);
