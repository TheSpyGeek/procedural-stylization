...
#ifndef ProceduralStyle05_H
#define ProceduralStyle05_H
...
class ProceduralStyle05Widget : public NodeWidget {
 public:
 ProceduralStyle05Widget(NodeInterface *node) :
  NodeWidget(node),
...
class ProceduralStyle05Node : public NodeTexture2D {
 public:
  ProceduralStyle05Node(PbGraph *parent=NULL,NodeHandle *handle=NULL);
  ~ProceduralStyle05Node();
  void apply();
...
class ProceduralStyle05Handle : public QObject, public NodeHandleInterface {
  Q_OBJECT
    Q_INTERFACES(NodeHandleInterface)
    Q_PLUGIN_METADATA(IID "Gratin.ProceduralStyle05")

    public:
  const QString     stringId() const {return "ProceduralStyle05Id";}
  unsigned int      version () const {return 1;}

  const QString     name    () const {return "proceduralStyle05";}
  ...
  NodeInterface *createInstance(PbGraph *parent) {
    return new ProceduralStyle05Node(parent,new NodeHandle(this));
  }
  ...
  NodeInterface *createInstance(PbGraph *parent,
    ...
    return new ProceduralStyle05Node(parent,new NodeHandle(stringId(),version(),name,path,desc,help,
						   inputNames,outputNames));
  }
};

#endif // ProceduralStyle05
