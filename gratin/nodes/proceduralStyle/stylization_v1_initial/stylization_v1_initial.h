...
#ifndef Stylization_v1_initial_H
#define Stylization_v1_initial_H
...
class Stylization_v1_initialWidget : public NodeWidget {
 public:
 Stylization_v1_initialWidget(NodeInterface *node) :
  NodeWidget(node),
...
class Stylization_v1_initialNode : public NodeTexture2D {
 public:
  Stylization_v1_initialNode(PbGraph *parent=NULL,NodeHandle *handle=NULL);
  ~Stylization_v1_initialNode();
  void apply();
 ...
 private:
  ...
  GPUProgram       _pSplat;
  GPUProgram       _pBlend;
  Stylization_v1_initialWidget *_w;
...
class Stylization_v1_initialHandle : public QObject, public NodeHandleInterface {
  Q_OBJECT
    Q_INTERFACES(NodeHandleInterface)
    Q_PLUGIN_METADATA(IID "Gratin.Stylization_v1_initial")

    public:
  const QString     stringId() const {return "stylization_v1_initialId";}
  unsigned int      version () const {return 1;}

  const QString     name    () const {return "stylization_v1_initial";}
  ...
  NodeInterface *createInstance(PbGraph *parent) {
    return new Stylization_v1_initialNode(parent,new NodeHandle(this));
  }
  ...
  NodeInterface *createInstance(PbGraph *parent,
    ...
    return new Stylization_v1_initialNode(parent,new NodeHandle(stringId(),version(),name,path,desc,help,
						   inputNames,outputNames));
  }
};

#endif // Stylization_v1_initial
